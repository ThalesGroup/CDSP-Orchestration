...
func (r *resourceCMSSHKey) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	id := uuid.New().String()
	tflog.Trace(ctx, MSG_METHOD_START+"[resource_cm_ssh_key.go -> Create]["+id+"]")

	// Retrieve values from plan
	var plan tfsdkCMSSHKeyModel
	var payload SSHKeyJSON
	...
	if err != nil {
		tflog.Debug(ctx, ERR_METHOD_END+err.Error()+" [resource_cm_ssh_key.go -> Create]["+id+"]")
	...
	response, err := r.client.PostDataBootstrap(ctx, id, URL_SSH_KEY, payloadJSON, "id")
	if err != nil {
		tflog.Debug(ctx, ERR_METHOD_END+err.Error()+" [resource_cm_ssh_key.go -> Create]["+id+"]")
		resp.Diagnostics.AddError(
	...
	tflog.Trace(ctx, MSG_METHOD_END+"[resource_cm_ssh_key.go -> Create]["+id+"]")
