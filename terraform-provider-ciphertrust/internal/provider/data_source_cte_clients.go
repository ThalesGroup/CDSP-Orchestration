...
type CTEClientsDataSourceModel struct {
	...
	Clients []tfsdkCTEClientsListModel `tfsdk:"clients"`
}
...
func (d *dataSourceCTEClients) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	id := uuid.New().String()
	tflog.Trace(ctx, MSG_METHOD_START+"[data_source_cte_clients.go -> Read]["+id+"]")
	...
	jsonStr, err := d.client.GetAll(
		ctx,
		id,
		URL_CTE_CLIENT+"/?"+strings.Join(kvs, "")+"skip=0&limit=10")

	if err != nil {
		tflog.Debug(ctx, ERR_METHOD_END+err.Error()+" [data_source_cte_clients.go -> Read]["+id+"]")
	...
	clients := []jsonCTEClientsListModel{}
	...
	for _, client := range clients {
		...
		clientState := tfsdkCTEClientsListModel{}
	...
	tflog.Trace(ctx, MSG_METHOD_END+"[data_source_cte_clients.go -> Read]["+id+"]")
