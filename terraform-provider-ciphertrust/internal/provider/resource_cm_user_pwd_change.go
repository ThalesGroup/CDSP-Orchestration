...
func (r *resourceCMPwdChange) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	id := uuid.New().String()
	tflog.Trace(ctx, MSG_METHOD_START+"[resource_cm_user_pwd_change.go -> Create]["+id+"]")

	// Retrieve values from plan
	var plan tfsdkCMPwdChangeModel
	var payload PwdChangeJSON
	...
	if err != nil {
		tflog.Debug(ctx, ERR_METHOD_END+err.Error()+" [resource_cm_user_pwd_change.go -> Create]["+id+"]")
	...
	response, err := r.client.PatchDataBootstrap(ctx, id, URL_CHANGE_USER_PWD, payloadJSON)
	if err != nil {
		tflog.Debug(ctx, ERR_METHOD_END+err.Error()+" [resource_cm_user_pwd_change.go -> Create]["+id+"]")
		resp.Diagnostics.AddError(
	...
	tflog.Trace(ctx, MSG_METHOD_END+"[resource_cm_user_pwd_change.go -> Create]["+id+"]")
...
func (d *resourceCMPwdChange) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	client, ok := req.ProviderData.(*CMClientBootstrap)
	if !ok {
		resp.Diagnostics.AddError(
