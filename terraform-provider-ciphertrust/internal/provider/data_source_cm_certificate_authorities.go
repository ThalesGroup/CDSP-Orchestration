...
type tfsdkCMCertificateAuthoritiesListModel struct {
...
type certificateAuthoritiesDataSourceModel struct {
	...
	CAs     []tfsdkCMCertificateAuthoritiesListModel `tfsdk:"cas"`
}
...
func (d *dataSourceCertificateAuthorities) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	id := uuid.New().String()
	tflog.Trace(ctx, MSG_METHOD_START+"[data_source_cm_certificate_authorities.go -> Read]["+id+"]")
	...
	jsonStr, err := d.client.GetAll(ctx, id, URL_LOCAL_CA+"/?"+strings.Join(kvs, "")+"skip=0&limit=10")
	if err != nil {
		tflog.Debug(ctx, ERR_METHOD_END+err.Error()+" [data_source_cm_certificate_authorities.go -> Read]["+id+"]")
	...
	for _, ca := range cas {
		caState := tfsdkCMCertificateAuthoritiesListModel{
	...
	tflog.Trace(ctx, MSG_METHOD_END+"[data_source_cm_certificate_authorities.go -> Read]["+id+"]")
